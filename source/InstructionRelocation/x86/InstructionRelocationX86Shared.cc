...
#define __ code_buffer->
  ...
  if (insn.primary_opcode >= 0x70 && insn.primary_opcode <= 0x7F) { // jcc rel8
    DEBUG_LOG("[x86 relo] {:p}: jc rel8", (void *)buffer_cursor);
    ...
    DEBUG_LOG("[x86 relo] {:p}: rip", (void *)buffer_cursor);
...
#define ___ rip_insn_seq_buffer.
      ...
      auto buffer_span = rip_insn_seq_buffer.GetBuffer();
      DobbyCodePatch((void *)rip_insn_seq_addr, buffer_span.data(), (uint32_t)buffer_span.size());
    }

  } else if (insn.primary_opcode == 0xEB) { // jmp rel8
    DEBUG_LOG("[x86 relo] {:p}: jmp rel8", (void *)buffer_cursor);
    ...
    DEBUG_LOG("[x86 relo] {:p}:jmp or call rel32", (void *)buffer_cursor);
  ...
  // insn -> relocated insn
  {
    ...
    DEBUG_LOG("insn -> relocated insn: {} -> {}", insn.length, relo_len);
  }
