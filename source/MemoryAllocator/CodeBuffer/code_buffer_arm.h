...
public:
  arm_inst_t LoadARMInst(uint32_t offset) {
    return *reinterpret_cast<arm_inst_t *>(GetBuffer().data() + offset);
  }

  thumb1_inst_t LoadThumb1Inst(uint32_t offset) {
    return *reinterpret_cast<thumb1_inst_t *>(GetBuffer().data() + offset);
  }

  thumb2_inst_t LoadThumb2Inst(uint32_t offset) {
    return *reinterpret_cast<thumb2_inst_t *>(GetBuffer().data() + offset);
  }

  void RewriteAddr(uint32_t offset, addr32_t addr) {
    memcpy(GetBuffer().data() + offset, &addr, sizeof(addr));
  }

  void RewriteARMInst(uint32_t offset, arm_inst_t instr) {
    *reinterpret_cast<arm_inst_t *>(GetBuffer().data() + offset) = instr;
  }

  void RewriteThumb1Inst(uint32_t offset, thumb1_inst_t instr) {
    *reinterpret_cast<thumb1_inst_t *>(GetBuffer().data() + offset) = instr;
  }

  void RewriteThumb2Inst(uint32_t offset, thumb2_inst_t instr) {
    memcpy(GetBuffer().data() + offset, &instr, sizeof(instr));
  }
