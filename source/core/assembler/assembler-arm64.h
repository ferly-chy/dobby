...
private:
  ...
  void LoadStore(LoadStoreOp op, CPURegister rt, const MemOperand &addr) {
    ...
    if (addr.IsImmediateOffset()) {
      // TODO: check Scaled ???
      imm12 = addr.offset() >> OpEncode::scale((uint32_t)LoadStoreUnsignedOffsetFixed | (uint32_t)op);
      Emit((uint32_t)LoadStoreUnsignedOffsetFixed | (uint32_t)op | LeftShift(imm12, 12, 10) | Rn(addr.base()) | Rt(rt));
  ...
  void LogicalShift(const Register &rd, const Register &rn, const Operand &operand, LogicalOp op) {
    ...
    Emit((uint32_t)op | (uint32_t)LogicalShiftedFixed | combine_fields_op);
  }
};
