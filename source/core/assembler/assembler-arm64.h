...
private:
  ...
  void LoadStore(LoadStoreOp op, CPURegister rt, const MemOperand &addr) {
    ...
    if (addr.IsImmediateOffset()) {
      // TODO: check Scaled ???
      imm12 = addr.offset() >> OpEncode::scale((int32_t)LoadStoreUnsignedOffsetFixed | (int32_t)op);
      Emit((int32_t)LoadStoreUnsignedOffsetFixed | (int32_t)op | LeftShift(imm12, 12, 10) | Rn(addr.base()) | Rt(rt));
  ...
  void LogicalImmediate(const Register &rd, const Register &rn, const Operand &operand, LogicalOp op) {
    ...
    Emit((int32_t)op | combine_fields_op);
  }
  void LogicalShift(const Register &rd, const Register &rn, const Operand &operand, LogicalOp op) {
    ...
    Emit((int32_t)op | (int32_t)LogicalShiftedFixed | combine_fields_op);
  }
};
